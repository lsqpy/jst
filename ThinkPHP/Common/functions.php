 ...
 * @return string
 */
function UR($url='',$vars='',$suffix=true,$redirect=false,$domain=false) {
    // 解析URL
    $info   =  parse_url($url);
    ...
    if($url) {
        ...
        /**/
        $routeflag = false;
        $urlRoute = C('URL_ROUTER_ON');
        $routeList = C('URL_ROUTE_RULES');
        
        
        if($urlRoute && $routeList){
            $count = substr_count($url,'/');
            if($count == 1){
                $url = GROUP_NAME.'/'.$url;
            }
            $key = array_search($url,$routeList);
            if(in_array($url,$routeList)){
                $url = $key;
            }
    ...
    }elseif(C('URL_MODEL') == 0) { // 普通模式URL转换
