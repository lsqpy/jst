...
class ShopAction extends CommonAction {
    ...
    public function index(){
        ...
        $map['status'] = 1;   //多篇文章
        $fields = array('id','name','price','hits','status','addtime');
        $data = $this->Shop->field($fields)->where($map)->order('id desc')->findPage();
        $this->assign($data);
        $this->display();
    }
    ...
    public function del(){
        ...
        $map['id'] = array('in',$id);
        $status = $this->Shop->where($map)->delete();
        if($status){
    ...
    public function search(){
        ...
        $list = $this->Shop->where($map)->order("id DESC")->findPage(30);
        $this->assign($list);
        $this->display('index');
    }
